...
* [ ] 同一用户可创建、切换、删除会话；标题自动/手动均可用
* [ ] 模型在**关闭记忆**时不引用任何持久化信息（通过审计与提示可视化验证）([OpenAI Help Center][2])
* [ ] 开启 **Project-only** 时，只注入项目作用域的偏好，且与全局不冲突或按优先级覆盖。([TechRadar][7])
* [ ] 超长对话时，**滚动摘要**能维持回答一致性；token 占用稳定
* [ ] 工具调用在参数缺失/超时/失败时有兜底（重试或降级答案）([OpenAI平台][5])
* [ ] 审计日志完整记录记忆读/写/删/覆盖事件，支持导出
...
* [ ] 初始化 FastAPI 项目结构
* [ ] 配置数据库（PostgreSQL 或 SQLite）
* [ ] 建立基础依赖（SQLAlchemy, Alembic 迁移, UUID 生成工具）

### 1.2 数据模型（Models）

* [ ] `Conversation` 表：`id, title, scope(project_id|null), created_at, updated_at`
* [ ] `Message` 表：`id, conversation_id, role, content, tool_calls(jsonb), created_at`
* [ ] `MemoryProfile`（全局/项目偏好）
* [ ] `MemoryEpisode`（会话记忆）
* [ ] `Summary`（滚动摘要）
* [ ] `AuditMemory`（审计日志）
...
* [ ] `POST /conversations` 新建会话（默认标题“新会话”）
* [ ] `PUT /conversations/{id}/title` 修改会话标题
* [ ] `GET /conversations` 获取会话列表
* [ ] `GET /conversations/{id}/messages` 获取消息列表
* [ ] `POST /conversations/{id}/messages` 添加用户消息 → 触发 AI 回复
* [ ] `DELETE /conversations/{id}` 删除会话

### 1.4 记忆 API

* [ ] `GET /memory/profile?scope=` 获取偏好记忆
* [ ] `POST /memory/profile` 写入偏好记忆
* [ ] `GET /memory/episode/{conversation_id}` 获取会话记忆
* [ ] `POST /memory/episode` 写入会话记忆
* [ ] `GET /audit/memory` 获取记忆审计记录
...
* [ ] 提取用户/助手消息中的“关键事实/术语/目标/约束”
* [ ] 判断是否持久化 → 写入 `MemoryEpisode` 或 `MemoryProfile`
* [ ] 添加审计日志（写入事件）
...
* [ ] 获取最近 K 条消息
* [ ] 获取最近一次摘要（Summary）
* [ ] 语义检索相关的会话记忆（Episode Memory）
* [ ] 加载相关的偏好（Profile Memory，作用域优先：Project > Global）
* [ ] 组装提示 → 返回给 LLM

### 2.3 滚动摘要

* [ ] 检测 token 数超阈值
* [ ] 调用 summarizer（内部 LLM）生成新摘要
* [ ] 存储到 `Summary` 表
* [ ] 清理过长历史，只保留近 K 条消息

### 2.4 生命周期管理

* [ ] 所有记忆允许 `expires_at` 字段
* [ ] 后台任务：定期清理过期记忆
* [ ] 审计日志记录删除事件
...
* [ ] `AppLayout`：左侧 Sidebar + 右侧 ChatWindow
* [ ] Sidebar 宽度 280px，深灰背景
* [ ] ChatWindow 白/浅灰背景
...
* [ ] 消息列表（User 气泡右对齐蓝色，Assistant 左对齐灰色）
* [ ] 输入框 + 发送按钮（蓝色，圆角）
* [ ] 支持多行输入（自动扩展高度）
* [ ] AI 回复显示打字机动画
* [ ] 滚动到最新消息
...
* [ ] **记忆开关**：关闭时，不引用持久化信息
* [ ] **工具调用**：正常触发 + 容错处理
* [ ] **治理**：用户可清空 / 导出记忆，审计日志可查
* [ ] **UI**：Sidebar + ChatWindow + Memory Center 三大区域可用
...
* 内容：
  ...
  * **注入的记忆片段**（Profile、Episode、Summary）
  * **Prompt 拼接结果**（最终发给 LLM 的上下文）
  * **工具调用轨迹**（调用参数 + 返回结果）
  * **Token 使用情况**（输入 / 输出 token）
* 顶部开关：

  * **启用 Debug 模式**（默认关闭，仅开发/测试时启用）

...
* **启动脚本**：
  * Windows: `start_dev.bat` - 自动检测、终止和启动开发服务
  * Unix/Linux/Mac: `start_dev.sh` - 提供同等功能的 bash 实现
* **服务管理功能**：
  * 自动检测端口占用（8000: 后端，5173: 前端）
  * 智能终止现有服务，避免端口冲突
  * 健康检查确认服务可用性
  * 优雅关闭确保资源释放
...
3. **环境管理**
   ...
   * 使用提供的启动脚本管理开发环境
     ...
     * Unix/Linux/Mac: `./start_dev.sh`
   * 脚本自动处理服务冲突和健康检查
   * 按 Ctrl+C 可优雅关闭所有服务
  
4. **文档驱动**
    * 在开始任务之前，请区分是前端开发，还是后端开发，你需要先查阅**docs** 相关文档，按照指定的开发要求进行任务编码
...
**Unix/Linux/Mac:**
```bash
chmod +x start_dev.sh  # 确保脚本有执行权限
./start_dev.sh
```

### 功能特性

- **自动检测服务:** 检测端口占用并终止现有服务
- **启动验证:** 确认服务成功启动并可访问
- **优雅关闭:** 使用 Ctrl+C 可一次性关闭所有服务
- **错误处理:** 提供详细错误提示和恢复选项
...
* 开发环境：使用 `start_dev.bat`（Windows）或 `start_dev.sh`（Unix/Linux/Mac）一键启动并管理服务